...
template <typename OD> void PdoObject<OD>::setCanId(uint32_t canId) {
  canId_ = canId;
}

template <typename OD> SdoErrorCode PdoObject<OD>::setActive() {
...
template <typename OD> void PdoObject<OD>::setInactive() { active_ = false; }

template <typename OD> bool PdoObject<OD>::isActive() const { return active_; }
...
template <typename OD> uint_fast8_t PdoObject<OD>::mappingCount() const {
...
template <typename OD> SdoErrorCode PdoObject<OD>::validateMappings() {
  return SdoErrorCode::NoError;
}
