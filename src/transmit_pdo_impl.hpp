...
template <typename OD> void TransmitPdo<OD>::sync() { sync_ = true; }

template <typename OD> void TransmitPdo<OD>::setValueUpdated() {
...
std::optional<modm::can::Message> TransmitPdo<OD>::nextMessage(Callback &&cb) {
  const bool send =
      (transmitMode_ == TransmitMode::OnSync && sync_) || sendOnEvent_.send();
...
SdoErrorCode TransmitPdo<OD>::setInhibitTime(uint16_t inhibitTime_100us) {
  sendOnEvent_.inhibitTime_ =
      std::chrono::microseconds(inhibitTime_100us * 100);
  return SdoErrorCode::NoError;
}

template <typename OD> uint16_t TransmitPdo<OD>::eventTimeout() const {
  return std::chrono::duration_cast<modm::Duration>(sendOnEvent_.eventTimeout_)
      .count();
}

template <typename OD> uint16_t TransmitPdo<OD>::inhibitTime() const {
