...
enum class TransmitMode { OnSync, OnEvent };

template <typename OD> class TransmitPdo;

template <typename OD>
modm::can::Message createPdoMessage(const TransmitPdo<OD> &pdo, uint16_t canId);

template <typename OD> class TransmitPdo : public PdoObject<OD> {
public:
  void sync();
...
protected:
  SdoErrorCode validateMapping(PdoMapping mapping) override;
};
